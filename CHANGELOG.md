...
- **架构决策记录 (Agent Notes)**：建立 `.agents/notes/` 目录规范，收录微内核架构设计、服务矩阵与插件生态集成、TUI 权衡与控制面重构深度分析、仓库精简与依赖解耦、Pi AgentHarness 工业级事务规格融合，以及**原生插件生态全景规划与 P0-P3 优先级演进矩阵**等中英文架构决策记录（ADR）；
