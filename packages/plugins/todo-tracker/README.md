...
Native Cordis task tracking and adaptive prompt injection plugin. It provides `todo_write` and `todo_read` tools supporting a 4-state task machine (`pending`, `in_progress`, `completed`, `cancelled`), with adaptive prompt compression that collapses finished tasks to save token budget.
...
When `injectToPrompt: true`, active non-completed tasks are listed in the prompt while completed/cancelled tasks are summarized into compact counts (`✓ N completed hidden`) to preserve conversation tokens.
