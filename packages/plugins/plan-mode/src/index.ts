...
const READ_ONLY_COMMAND_PATTERNS = [
	...
	/^\s*(?:get-content|get-childitem|get-item|get-location|get-command|get-process|select-string|resolve-path|test-path|measure-object|compare-object|format-list|format-table)\b/i,
];
...
export function apply(ctx: Context, config: PlanModeConfig = {}) {
	...
	const unregisterCommand = ctx.extensions.registerCommand("plan", {
		description: "Enter Plan mode, or leave it with /plan off",
		...
		handler: (args: string, commandContext: PiExecutionContext) => {
			...
			if (requested && requested !== "on" && requested !== "off") {
				throw new Error(`Unknown Plan mode argument "${requested}". Use /plan or /plan off.`);
			}
			const active = requested !== "off";
			setActive(sessionId, active);
			commandContext.ui?.notify?.(
				active ? "Plan mode enabled. Workspace mutations are blocked." : "Plan mode disabled.",
				"info",
			);
		},
	});
	...
	const unregisterTool = ctx.tools.register({
		...
		renderCall: (args: { plan?: string }) => args.plan?.match(/^#\s+(.+)$/m)?.[1] ?? "Review plan",
		renderResult: (toolResult: PlanToolResult) => toolResult.content[0]?.text ?? "Plan review finished",
		execute: async (args: { plan: string }, executionContext?: ToolExecutionContext): Promise<PlanToolResult> => {
			...
			const validationError = validatePlanMarkdown(args.plan ?? "");
			if (validationError) {
			...
			if (!piContext?.hasUI || !ui?.select) {
				return result(
					...
					{ isError: true, plan: args.plan.trim() },
				);
			}
			...
			if (choice === "Approve and leave Plan mode") {
				state.lastApprovedPlan = args.plan.trim();
