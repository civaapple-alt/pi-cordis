...
- `allowedCommands`（string[]，可选）：可绕过模式检查的完整命令字符串；后缀和复合命令不会继承例外。

## 多层级安全防护引擎

1. **高危命令正则分析**：
   - 根目录/用户目录误删：`rm -rf /`、`rm -rf /*`、`rm -rf ~`、`rm -rf $HOME`、`rm -rf ..`；
   - 磁盘格式化与块设备覆盖：`mkfs`、`dd if=`、`> /dev/sda`、`> /dev/nvme0n1`；普通空设备重定向（如 `2>/dev/null`）保持允许；
   - Fork 炸弹：`:(){ :|:& };:`；
   - 越权权限开放：`chmod -R 777 /`；
   - 远程脚本直接执行：`curl ... | bash`；
   - 敏感凭证打印：`cat .env`、`cat ~/.ssh/*`；
   - Windows 根路径删除与格式化：高危形式的 `Remove-Item`、`del`/`rd`、`format` 与 `Clear-Disk`。
2. **敏感密钥与文件保护**：阻止对 `.env`、`.git/`、`.ssh/`、`*.pem`、`id_rsa` 的写入与覆盖。
3. **只读模式强拦截**：在 Plan 或审查等只读阶段自动阻断破坏性修改。

## 模型体验
- **前置防护栏**：在潜在危险命令执行前立即拦截并返回友好的策略说明。
- **默认覆盖**：默认启用在 `default` 与 `ptc`；独立 Plan 状态另行叠加自身只读策略。

该插件是针对常见误操作的纵深防护栏。基于模式的匹配不是安全沙箱，不能作为不可信代码的隔离边界。
